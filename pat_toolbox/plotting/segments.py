...
def _add_segment_pages_to_pdf(
    ...
    *,
    ...
    for start in range(0, n_samples, samples_per_segment):
        ...
        n_rows = (1 if use_hr else 0) + (1 if use_prv else 0) + (1 if use_prv_sdnn else 0)
        if n_rows == 0:
            continue
        ...
        _overlay_events_on_axes(aux_df, seg_start_sec, seg_end_sec, ax_hr=ax_hr, ax_prv=ax_prv if use_prv else None, ax_prv_sdnn=ax_prv_sdnn if use_prv_sdnn else None, hr_ylim=hr_ylim, prv_ylim=prv_ylim, prv_sdnn_ylim=prv_sdnn_ylim, event_spec=event_spec)

        if ax_prv_sdnn is not None:
